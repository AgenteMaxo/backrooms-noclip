...
const catalog = require(path.join(__dirname, '..', 'data', 'game', 'levels.es.json'));
// El catálogo completo tiene su propio explorador (`make-catalog-map.js`).
// Este archivo conserva el conjunto reducido que usa upstream v28.9 para que
// `mapa-piloto.html` no se convierta accidentalmente en otro mapa de 731 nodos.
const PILOT_IDS = [
  'level-0', 'level-1', 'level-2', 'level-6', 'level-13', 'level-14', 'level-15', 'level-16',
  'level-27', 'level-45', 'level-80', 'level-130', 'level-140', 'level-144', 'level-186',
  'level-188', 'level-268', 'level-305', 'level-306', 'level-385', 'level-399', 'level-483',
  'level-483-5', 'level-484', 'level-626', 'level-777', 'level-909', 'level-995', 'level-996',
  'the-hub', 'level-6-1', 'level-7', 'level-8', 'level-11', 'level-24', 'level-37',
  'level-203', 'level-307', 'level-499', 'level-833', 'level-910',
];
const levels = Object.fromEntries(PILOT_IDS.filter((id) => catalog[id]).map((id) => [id, catalog[id]]));
